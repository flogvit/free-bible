...
const runs = files.map(f => JSON.parse(readFileSync(join(VERDICTS, f), 'utf8')));
...
const BANDS = [[0.15, 0.25], [0.25, 0.40], [0.40, 0.60], [0.60, 1.01]];
const label = ([lo, hi]) => `${(lo * 100).toFixed(0)}-${Math.min(100, hi * 100).toFixed(0)} %`;

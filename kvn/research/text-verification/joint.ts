...
const mad = (a, m) => 1.4826 * median(a.map(x => Math.abs(x - m))) || 1e-6;

// --- referansefordeling bygges på HELE testsettets OK-rader, ikke bare utvalget ---
const BANDS = [[0, 80], [80, 160], [160, 320], [320, 1e9]];
const bandOf = len => BANDS.findIndex(([lo, hi]) => len >= lo && len < hi);

const ref = new Map();   // tr|band → {lm, ls, sm, ss}
for (const r of rows) {
...
const scored = cases.map(c => ({ ...c, z: score(c) })).filter(c => c.z);
...
for (const FA of [0.01, 0.03]) {
  for (const [name, f] of Object.entries(VARIANTS)) {
    ...
    const pc = k => { const s = verd.filter(x => x.kind === k); return s.length ? `${(100 * s.filter(x => x.flag).length / s.length).toFixed(0)}%` : '-'; };

...
const embed = async t => {
  ...
  return (await r.json()).embeddings.map(v => { let s = 0; for (const x of v) s += x * x; s = Math.sqrt(s) || 1; return v.map(x => x / s); });
};
...
const mad = (a, m) => 1.4826 * median(a.map(x => Math.abs(x - m))) || 1e-9;
...
const rows = [];   // {tr, kind, sim, len}  kind: OK | GRENSE | FEILVERS | FLETTET | AVKORTET
...
for (const tr of TR) {
  ...
  for (const [b, c] of CH) {
    ...
    if (!os || !t) continue;
    const tById = new Map(t.map(v => [v.verseId, v.text]));
    const osById = new Map(os.map(v => [v.verseId, v.text]));
...
function calib(get, dir, faTarget) {
...
for (const fa of [0.01, 0.05]) {
  ...
  for (const [name, get, dir] of DET) {
    ...
    const hit = r => dir < 0 ? get(r) < th : get(r) > th;
  ...
  const ths = DET.map(([, get, dir]) => [get, dir, calib(get, dir, fa / DET.length)]);
  const any = r => ths.some(([get, dir, th]) => dir < 0 ? get(r) < th : get(r) > th);

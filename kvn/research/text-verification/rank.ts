...
import type { Chapter, Verse } from '../../src/bible-types.js';
...
/** Et osmain-kapittel med versene sine embeddet én gang. */
...
/** Et par med rangeringen sin blant kapitlets osmain-vers. */
interface Ranked extends Case {
  rank: number | null;
  margin: number | null;
}

/** Svaret fra Ollamas /api/embed. */
interface EmbedResponse {
  embeddings: number[][];
}

/** En navngitt flaggregel over de rangerte parene. */
type Variant = [name: string, flag: (r: Ranked) => boolean | null];
...
const embed = async (texts: string[]): Promise<Float32Array[]> => {
  ...
  return ((await r.json()) as EmbedResponse).embeddings.map(v => {
...
for (const [b, c] of CH) {
  ...
  const verses = (JSON.parse(readFileSync(f, 'utf8')) as Chapter).filter(v => v.text?.length > 20);
...
const byText = new Map<string, { ch: EmbeddedChapter; i: number }>();
...
for (const [name, f] of variants) {
  ...
  const pc = (k: string): string => { const s = verd.filter(x => x.kind === k && x.flag !== null); return s.length ? `${(100 * s.filter(x => x.flag).length / s.length).toFixed(0)}%` : '-'; };

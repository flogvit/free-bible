...
moduler til **1 feilende vers og 1157 av 1158 rene**.
...
- `basque` sin Sal 110 pekte på osmain 110, men modulens Sal 110 er osmain 111.
  Fire salmer i den modulen var koblet til feil kapittel.
...
at **190 300 mappingposter kunne fjernes** — de europeiske modulene trenger
...
slutning. At 600+ moduler har en ombrytningsblokk i samme kapittel er
...
Rettingen gjorde at **4160 modul-salmer ikke trenger poster i det hele tatt**
og 1008 får den vanlige hebraiske forskyvningen.

## 2. Feil i modulenes kildedata
...
| `albanian` | 3 Joh, Judas og Åp 1–12 er forskjøvet over bokgrensene. Modulens 3 Joh 1,14 **er** Judas 1,1, og dens Judas 1,25 er Åp 1,1. Åp 13–22 er riktige — forskyvningen løser seg opp fordi albansk Åp 12 har 17 vers mot osmains 18. |
...
- **`***` som plassholder** for vers høstingen ikke fikk. 65 forekomster i
  `english_darby` alene, og i mange andre moduler. De har ingen motpart i osmain.
- **Moduler som bærer begge nummereringene.** `kvn`-referanseteksten,
...
| Manglende grensepost | Modulens første vers hører til forrige kapittel | `kvn`: 14 hebraiske kapittelgrenser |
| Manglende delvers | Modulen deler eller fletter et osmain-vers | `spanish`: Dom 14,18 delt i to |
| Feil kapittelkobling | LXX-modul koblet mot sitt eget nummer | `basque`: Sal 110 mot osmain 110 |
...
- **Siste versnummer, ikke antallet**, skiller hull i høstingen fra hebraisk
  forskyvning. En modul med 28 vers i 5 Mos 29 kan være begge deler; `max(verseId)`
  avgjør. 115 modul-kapitler hadde fått forskyvning de ikke skulle ha før dette
  ble skilt.
- **Identisk versstruktur som osmain ⇒ identitet.** 942 modul-kapitler hadde
...
- **Automatisk kapitteljustering på versantall.** En DP-justering av hele
  salteret mot osmain ble prøvd på de 34 modulene som feilet der; kostnaden lå
  mellom 25 og 142 for samtlige, og den ble avvist for alle. Moduler med hull i
...
- **Semantisk verifisering er ikke uttømmende.** Feilklassen «bijektiv, men peker
  på feil vers» ble funnet ved lesing i de modulene som feilet på tall. De 1157
  rene modulene er ikke gjennomlest. Ordensbrudd-sjekken (punkt 4) er den

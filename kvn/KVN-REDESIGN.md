...
The current KVN system uses osnb2 (which follows Hebrew/Tanach numbering) as the "basis" coordinate system. Other translations are mapped relative to osnb2. This creates problems:

- **Extra verses**: Verses that exist in a translation but not in osnb2 (e.g., Joel 3:6-26, Rom 16:25-27, Neh 7:68) need special handling as "extraVerses"
- **Collisions**: When two osnb2 verses map to the same translation verse (e.g., 4 Mos 25:19 + 26:1 → b2011 26:1), the system needs collision detection
- **Basis-centric**: The whole system breaks down if a translation has content that osnb2 doesn't cover
...
3. **Data collection**: Need enough translations to discover all numbering variants. Key sources:
   - Norwegian: osnb2, Bibel 2011, NB88, etc.
...
- **826 map entries** (non-identity verse number differences between osnb2 and Bibel 2011)
- **25 extra verses** (b2011-only: 1 Neh 7:68, 21 Joel 3:6-26, 3 Rom 16:25-27)
- All verified against actual Bible text content (both osnb2 and dnb2011_nb.txt)
- All 154 tests passing across 6 test suites

### Mapping patterns discovered:
1. **24 paired chapter boundary shifts** - adjacent chapters where b2011 and osnb2 split the chapter boundary differently (e.g., Hebrew Exodus 7 has 29 verses while European has 25, with the extra 4 being the start of European chapter 8)
...
4. **Cross-chapter collision** - 4 Mos 25:19 → b2011 26:1 (osnb2 has a short verse that b2011 merges into the next chapter's first verse)
...
- `generate/bibles_raw/osnb2/` - osnb2 verse data (JSON per chapter)
...
1. Decide on the universal KVN approach vs keeping osnb2 as basis

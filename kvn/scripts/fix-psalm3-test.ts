 ...
 *
 * Send osnb verses + a reference bible (ESV) to Ollama.
 * Ask it to map each ESV verse to the matching osnb verse(s).
 ...
 * dem. Alt arbeidet ligger i `main()` slik at hjelpen kommer ut før skriptet
 * leser bibeldata eller laster en modell i Ollama.
 */
...
const SPEC: Record<string, FlagSpec> = {
  save: {kind: 'boolean', help: 'skriv det gjenoppbygde kapittelet til osmain 19/3.json'},
    ...
    mappings: {
      ...
      items: {
        ...
        properties: {
          ...
          osnbVerses: { type: 'array' as const, items: { type: 'number' as const }, description: 'Which osnb verse(s) make up this verse' },
...
async function main() {
  ...
  if (flags.help) {
    console.log(formatHelp(
      ...
      'lar Ollama mappe osnb Sal 3 mot ESV/DNB2011-nummereringen og bygger kapittelet på nytt',
      SPEC,
      [
        'bun kvn/scripts/fix-psalm3-test.ts',
        'bun kvn/scripts/fix-psalm3-test.ts --save',
      ],
    ));
    process.exit(0);
  }
  ...
  // Load data
  const osnb = loadChapter(OSNB_DIR, 19, 3);
  ...
  console.log('=== OSNB Sal 3 ===');
  for (const v of osnb) console.log(`  v${v.verseId}: ${v.text.slice(0, 80)}`);
...
For each target verse number (1-${esv.length}), tell me which osnb verse(s) it corresponds to.
...
Compare the CONTENT of the verses to determine the mapping. Do not just match by verse number.`;
  ...
  for (const m of result.mappings) {
    ...
    console.log(`  target v${m.targetVerse} ← osnb v${osnbRefs} [${m.mergeType}]`);
  }

/**
 * Build a universal KVN mapping for osnb2.
 *
 ...
 *
 * For osnb2, most coordinates map directly (identity).
 * We also compare against the raw JSON bibles to find where osnb2 differs.
 */
...
const OUT_FILE = join(import.meta.dirname, '../mappings/osnb2.ukvn.json');
...
const osnb2 = loadOsnb2();

console.log(`Loaded ${osnb2.size} chapters from osnb2`);

// Collect all verse positions
let totalVerses = 0;
const allPositions: { book: number; chapter: number; verse: number; kvn: number }[] = [];

for (const [key, verses] of osnb2) {
...
console.log(`Total verses in osnb2: ${totalVerses}`);
...
// === Build mapping file ===
// For osnb2, the mapping is identity — its coordinates ARE KVN coordinates.
...
const mapping: {
  ...
  // Verse number differences: tkvn -> kvn (empty for osnb2, it IS the basis)
...
console.log(`Map entries (tkvn≠kvn): ${mapping.map.length} (identity for osnb2)`);

 ...
 *   bun scripts/verify-text-report.ts --help             # hele flaggtabellen
 */
...
const SPEC: Record<string, FlagSpec> = {
  class: {kind: 'string', help: 'bare én feilklasse: WRONG | MERGED | SHORT | UNRESOLVED'},
  list: {kind: 'boolean', help: 'skriv ut verstekstene, ikke bare sammendraget'},
  limit: {...COMMON_FLAGS.limit, help: 'hvor mange vers --list skriver ut', default: 25},
...
if (flags.help) {
  console.log(formatHelp(
    ...
    'arbeidsliste fra tekstverifiseringen, sortert etter feilklasse',
    SPEC,
    [
      'bun kvn/scripts/verify-text-report.ts                    # sammendrag, alle',
      'bun kvn/scripts/verify-text-report.ts kjv spanish        # bare disse',
      'bun kvn/scripts/verify-text-report.ts --class WRONG      # bare én klasse',
      'bun kvn/scripts/verify-text-report.ts --list --limit 40  # med verstekst',
    ],
  ));
  process.exit(0);
}
...
const WANT_CLASS = flags.class as string | undefined;

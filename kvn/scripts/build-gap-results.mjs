...
const only = (process.argv.indexOf('--only') >= 0 ? process.argv[process.argv.indexOf('--only') + 1] : null)?.split(',');
const verses = p => JSON.parse(fs.readFileSync(p, 'utf8')).map(v => v.verseId);

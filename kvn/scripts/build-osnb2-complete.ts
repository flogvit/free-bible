/**
 * Build osnb2_complete - complete Bible with all verse positions.
 *
 * Creates generate/bibles_raw/osnb2_complete/ with all 66 books + extra chapters,
 * filling in ~143 verse positions that exist in ≥50 other translations but not in osnb2.
 *
 * Also writes kvn/data/osnb2-additions.json with metadata for each added verse.
 *
 * Run: npx tsx kvn/scripts/build-osnb2-complete.ts
 */
...
const OSNB2_DIR = join(PROJECT_ROOT, 'generate/bibles_raw/osnb2');
...
const ADDITIONS_FILE = join(__dirname, '../data/osnb2-additions.json');
...
// ============================================================
// Step 1: Read all osnb2 verses
// ============================================================

console.log('Step 1: Reading osnb2 verses...');

const osnb2Data = new Map<string, RawVerse[]>();
const osnb2Set = new Set<string>();
let osnb2VerseCount = 0;

for (let book = 1; book <= 66; book++) {
  const bookDir = join(OSNB2_DIR, String(book));
  ...
  for (const file of files) {
    ...
    osnb2Data.set(`${book}-${chapter}`, verses);
    for (const v of verses) {
      osnb2Set.add(`${book}-${chapter}-${v.verseId}`);
      osnb2VerseCount++;
    }
  }
}

console.log(`  Found ${osnb2VerseCount} osnb2 verses`);
...
for (const [bookIdStr, book] of Object.entries(report.books)) {
  ...
  for (const [chapterStr, chapter] of Object.entries(book.chapters)) {
    ...
    for (let verseId = 1; verseId <= maxVerse; verseId++) {
      if (osnb2Set.has(`${bookId}-${chapterId}-${verseId}`)) continue;
...
// Map from European/tkvn key → osnb2/kvn key (only where different and osnb2 exists)
...
// These are well-documented differences where the text exists in osnb2
...
const manualShifts: Array<{ eurBook: number; eurChapter: number; eurVerse: number; hebChapter: number; hebVerse: number }> = [
  ...
  // Joel 2:28-32 (European) = Joel 3:1-5 (Hebrew/osnb2)
  ...
  // Job 41:27-34 (ASV/European) = Job 41:19-26 (Hebrew/osnb2) — offset -8
  ...
  // Psalm 11:8 (European/Septuagint) = Psalm 12:9 (Hebrew/osnb2)
  { eurBook: 19, eurChapter: 11, eurVerse: 8, hebChapter: 12, hebVerse: 9 },
  // Psalm 43:6 (European/Septuagint) = Psalm 44:7 (Hebrew/osnb2)
  { eurBook: 19, eurChapter: 43, eurVerse: 6, hebChapter: 44, hebVerse: 7 },
  // Psalm 70:7 (European/Septuagint) = Psalm 71:7 (Hebrew/osnb2)
  { eurBook: 19, eurChapter: 70, eurVerse: 7, hebChapter: 71, hebVerse: 7 },
  // Psalm 127:6 (European/Septuagint) = Psalm 128:6 (Hebrew/osnb2)
  { eurBook: 19, eurChapter: 127, eurVerse: 6, hebChapter: 128, hebVerse: 6 },
  // Joel 4:1-5 (Hebrew 4-chapter) = Joel 3:1-5 (Hebrew 3-chapter/osnb2)
...
for (const s of manualShifts) {
  ...
  if (!reverseMapping.has(eurKey) && osnb2Set.has(hebKey)) {
...
// Verse splits: positions where the content is part of an existing osnb2 verse.
...
for (const pos of missingPositions) {
  ...
  // Try 1: Reverse mapping (numbering shift — text exists in osnb2 under different number)
  const osnb2Source = reverseMapping.get(key);
  if (osnb2Source) {
    const [sBook, sChapter, sVerse] = osnb2Source.split('-').map(Number);
    const chapterVerses = osnb2Data.get(`${sBook}-${sChapter}`);
  ...
  // Try 2: Verse splits (text is part of an existing osnb2 verse)
  ...
  if (split) {
    addedVerses.set(key, {
      ...
      verseId: pos.verseId,
      text: split.text,
      source: `osnb2:${split.fromChapter}:${split.fromVerse}`,
    });
    additions.push({ ...pos, source: `osnb2:${split.fromChapter}:${split.fromVerse}`, type: 'numbering_shift' });
    numberingShifts++;
    continue;
  }
...
// ============================================================
// Step 5: Build osnb2_complete and write files
// ============================================================

console.log('Step 5: Writing osnb2_complete...');

const completeData = new Map<string, OutputVerse[]>();

// Copy all osnb2 verses (stripping versions field)
for (const [chapterKey, verses] of osnb2Data) {
...
console.log('\n=== Summary ===');
console.log(`  osnb2 verses:      ${osnb2VerseCount}`);
console.log(`  Added verses:      ${additions.length}`);
console.log(`  Total verses:      ${totalVerseCount}`);
console.log(`  Numbering shifts:  ${numberingShifts} (Norwegian from osnb2)`);

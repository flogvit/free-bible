 ...
 * dem. Hele jobben ligger i `main()` nettopp fordi den leser hele osmain ved
 * oppstart — som toppnivåkode ble `--help` en full skanning.
 */
 ...
 *
 * Skriptet filtrerer på nøkkelen `bok:kapittel` («19:3»), ikke på et
 * kapittelintervall. Som `range` ville «19:3» blitt tolket som kapittel 19 av
 * `parseInt`, og filteret hadde stille sluttet å treffe noe.
 */
const SPEC: Record<string, FlagSpec> = {
  verify: {kind: 'boolean', help: 'kjør Ollama-verifiseringen; uten den bare skannes det'},
...
const HELP_EXAMPLES = [
  'bun kvn/scripts/verify-osmain.ts',
  'bun kvn/scripts/verify-osmain.ts --verify',
  'bun kvn/scripts/verify-osmain.ts --verify --bible english_esv',
  'bun kvn/scripts/verify-osmain.ts --verify --chapter 19:3',
];
...
async function main(): Promise<void> {
  ...
  if (flags.help) {
    console.log(formatHelp(
      ...
      'sammenlikner osmains versnummerering med de rå biblene og kan la Ollama forklare avvikene',
      SPEC,
      HELP_EXAMPLES,
    ));
    process.exit(0);
  ...
  // Begge filtrene var `null` når de manglet, ikke `undefined`.
  const bibleFilter = (flags.bible as string | undefined) ?? null;

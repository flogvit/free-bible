...
const HELP_PURPOSE =
    'kapittel-spesifikk historisk og kulturell kontekst for bibelkapitler. ' +
    'Kjør book-context.ts først — bok-konteksten leses inn som bakgrunn, og ' +
    'uten den gjentar kapittelteksten det som hører hjemme på boknivå.';

const HELP_EXAMPLES = [
    'bun generate/chapter-context.ts --nt                         # NT, norsk bokmål',
    'bun generate/chapter-context.ts --language nn --ot           # GT, nynorsk',
    'bun generate/chapter-context.ts --language en --book 43      # Johannes, engelsk',
    'bun generate/chapter-context.ts --book 1 --chapter 1-11      # 1. Mosebok 1-11',
    'bun generate/chapter-context.ts --nt --proofread --apply     # generer → korrektur → skriv inn',
    'bun generate/chapter-context.ts --local --book 10-39         # lokal Ollama i stedet for Claude',
    'bun generate/chapter-context.ts --book 1 --force             # generer 1. Mosebok på nytt',
    '',
    'Filene havner i chapter_context/<språkkode>/<bok>-<kapittel>.md,',
    'f.eks. chapter_context/nb/1-1.md (1. Mosebok 1).',
    '',
    'Parallellkjøring i hver sin terminal:',
    '  bun generate/chapter-context.ts --book 1-20 &',
    '  bun generate/chapter-context.ts --book 21-39 &',
];
...
if (flags.help) {
    console.log(formatHelp('generate/chapter-context.ts', HELP_PURPOSE, SPEC, HELP_EXAMPLES));
    process.exit(0);
}
...
function readOptions(parsed: typeof flags): Options {
    ...
    const chapter = parsed.chapter as Range | undefined;

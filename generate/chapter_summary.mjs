...
function fileExists(language, bookNr, chapterNr) {
    const filePath = path.join(__dirname, `summaries/${language}/${bookNr}-${chapterNr}.txt`);
    return fs.existsSync(filePath) && fs.statSync(filePath).size > 0;
}

async function generateChapterSummary(language, bookId, chapter) {
    const bookName = bookNames[bookId];
    const bibleRef = `${bookName} ${chapter}`;

    const prompt = `Lag et sammendrag av ${bibleRef} på norsk, bokmål.`;

    console.log(`Generating chapter summary for ${bibleRef}...`);
    const completion = await doAnthropicCall(prompt);
    const text = completion.content[0].text;

    const outputDir = path.join(__dirname, `summaries/${language}`);
...
async function main() {
    ...
    for (let bookId = startBook; bookId <= 66; bookId++) {
        ...
        for (let chapter = firstChapter; chapter <= maxChapters; chapter++) {
            ...
            await generateChapterSummary(language, bookId, chapter);
        }
    }
}

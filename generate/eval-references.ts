 ...
 * ordrett videre til `references-semantic` og lagres i `eval/results.json` som
 ...
 * `bun generate/eval-references.ts --help` startet en full LLM-evaluering (#109).
 *
 * `--threshold` er `string` av samme grunn som i `references-semantic`:
...
const HELP_EXAMPLES = [
    'bun generate/eval-references.ts',
    'bun generate/eval-references.ts --top-k 10 --threshold 0.78',
    'bun generate/eval-references.ts --concepts --theme',
    'EVAL_VERSES=extra bun generate/eval-references.ts',
    '',
...
async function runPipelineOnVerse(verse: TestVerse, extraArgs = '') {
    ...
    const cmd = `bun ${path.join(__dirname, 'references-semantic.ts')} --verify-only --book ${verse.bookId} --chapter ${verse.chapterId} --verse ${verse.verseId} ${extraArgs}`;
    const t0 = Date.now();
    let output = '';
    try {

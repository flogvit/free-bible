...
**Two documents, and the split between them is the point.** `STATUS.md` says *what is
missing* — it is generated by `generate/build-status.ts` from the data itself, because
hand-maintained coverage numbers were wrong the same day they were written (the old
`lokale-jobber.md` said 911 of 1,189 chapters while the number was 1,178, and issue
titles still say 301). Never write a coverage number into prose; regenerate instead.
`docs/running-jobs.md` says *how to run* — models, memory, adoption, concurrency — and
...
- **Two kinds of test, two places.** A test of a module lives next to it
  ...
  a directory is renamed: the first moves with its module, the second does not move
  at all. `tests/` is on `bun test`'s path, so a guard put there runs; that is the
  whole point of the split, since a test nobody discovers passes silently.
- `bun test` scans the whole tree, with `external/`, `generate/bibles_raw/`,
  `node_modules/` and `kvn/` cut out in `bunfig.toml` (`pathIgnorePatterns`). Do not
  narrow `root` to one directory instead — that hides every test outside it. `kvn/`
  is excluded because it has its own suite and tsconfig: `bun run test:all` runs both.
- **Every filename the docs name must exist**, and `tests/docs-filnavn.test.ts`
  enforces it. The `.mjs`→`.ts` rename left CLAUDE.md pointing at nine files that were
  gone and `docs/running-jobs.md` at nine more — including `bun triage.mjs osnb`, a
